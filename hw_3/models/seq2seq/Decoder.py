...
class Decoder(nn.Module):
    ...
    def __init__(self, emb_size, encoder_hidden_size, decoder_hidden_size, output_size, dropout=0.2, model_type="RNN", attention=False):
        ...
        if model_type == "RNN":
            ...
            self.rnn = nn.LSTM(emb_size, decoder_hidden_size)
        self.dropout = nn.Dropout(dropout)
        if attention:
            self.attn = nn.Linear(encoder_hidden_size + emb_size, emb_size)
    ...
    def compute_attention(self, hidden, encoder_outputs):
        ...
        attention_prob = cosine_sim(hidden, encoder_outputs).unsqueeze(1)
        attention_prob = torch.softmax(attention_prob, dim=2)
    ...
    def forward(self, input, hidden, encoder_outputs=None):
        ...
        if self.attention:
            if self.model_type == "LSTM":
                ...
                hidden, c_s = hidden, None

            attn_weights = self.compute_attention(hidden, encoder_outputs)
            context = torch.bmm(attn_weights, encoder_outputs)
            context = context.permute(1, 0, 2)
            rnn_input = self.attn(torch.cat((embedded, context), dim=2))

            if self.model_type == "LSTM":
                rnn_hidden = (rnn_input, c_s)
            else:
                rnn_hidden = rnn_input
        else:
            rnn_input = embedded
            rnn_hidden = hidden

        output, hidden = self.rnn(rnn_input, rnn_hidden)
        output = torch.log_softmax(self.out(output.squeeze(0)), dim=1)
