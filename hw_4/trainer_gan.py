...
class GANTrainer(Trainer):
    ...
    def compute_loss_real(self, data, batch_size):
        ...
        loss = self.criterion(outputs, real_labels)
        return loss
    ...
    def compute_loss_fake(self, batch_size, latent_dim):
        ...
        #############################################################################
        fake_labels = torch.full((batch_size, 1), 0.0, device=self.device)
        z = torch.randn((batch_size, latent_dim), device=self.device)
        fake_images = self.generator(z).detach()
        outputs = self.discriminator(fake_images)
        loss = self.criterion(outputs, fake_labels)
        return loss
       ...
       # loss_meter = AverageMeter()
        ...
        for epoch in range(self.n_epochs):
            ...
            for i, (data, _ ) in enumerate(self.train_loader):
                ...
                loss_real = self.compute_loss_real(data.view(batch_size, -1), batch_size)
