...
class DiffusionTrainer(Trainer):
    ...
    def forward_diffusion(self, x_0, t):
        ...
        mu = torch.sqrt(self.alphas_bar[t]).view(batch_size,1,1,1) * x_0
        epsilon = torch.sqrt(1 - self.alphas_bar[t]).view(batch_size,1,1,1) * noise
        x_t = mu + epsilon
        ...
        for epoch in range(self.n_epochs):
            ...
            for i, (data, _ ) in enumerate(self.train_loader):
               ...
               #############################################################################
                ...
                noisy_samples, noise = self.forward_diffusion(data, t)
                out = self.net(noisy_samples, t.unsqueeze(1))
    ...
    def sample_timestep(self, x, t):
        ...
        alphas_bar = self.alphas_bar[t].view(batch_size,1,1,1)
        mu_x_prev = (1 / torch.sqrt(alpha_t)) * (x - ( (1 - alpha_t) / torch.sqrt(1 - alphas_bar)) * predicted_noise)
        ...
        if (t == 0).all():
            ...
            x_prev = mu_x_prev + sigma_t * noise
        #############################################################################
        #                              END OF YOUR CODE                             #
        #############################################################################
        return x_prev
