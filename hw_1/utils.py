...
def load_mnist_trainval():
    ...
    num_elements = int(len(data) * 0.8)
    train_data = data[:num_elements]
    train_label = label[:num_elements]
    val_data = data[num_elements:]
    val_label = label[num_elements:]
...
def plot_curves(train_loss_history, train_acc_history, valid_loss_history, valid_acc_history):
    ...
    plt.xlabel('Epochs')
    plt.ylabel('Loss')
    plt.title('Training and Validation Loss (Optiaml)')
    plt.legend()
